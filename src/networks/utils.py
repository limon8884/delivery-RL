...
def create_mask(lenghts, device, mask_first) -> torch.BoolTensor:
    ...
    with torch.no_grad():
        ...
        result = (
            torch.arange(max_len, device=device).expand(len(lenghts), max_len)
            >=
            torch.tensor(lenghts, device=device).unsqueeze(1)
            )
        if mask_first:
            result[:, 0] = True
        return result
...
def get_batch_masks(triples: List[GambleTriple], device):
    return {
        'o': pad_sequence([torch.BoolTensor([True] + [False] * len(triple.orders)) for triple in triples],
                          batch_first=True, padding_value=True).to(device=device),
        'c': pad_sequence([torch.BoolTensor([True] + [False] * len(triple.couriers)) for triple in triples],
                          batch_first=True, padding_value=True).to(device=device),
        'ar': pad_sequence([torch.BoolTensor([True] + [False] * len(triple.active_routes)) for triple in triples],
                           batch_first=True, padding_value=True).to(device=device)
    }
...
def get_cross_mask(masks) -> torch.FloatTensor:
    ...
    with torch.no_grad():
        ...
        fake_part_mask = torch.zeros((inverse_binary_mask.shape[0], inverse_binary_mask.shape[1], 1),
                                     device=inverse_binary_mask.device)  # [bs, o, 1]
