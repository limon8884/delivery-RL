...
class ClaimAttention(nn.Module):
    def __init__(self, **kwargs):
        ...
        self.fc = nn.Linear(self.clm_emb_dim + 2, self.clm_emb_dim).to(self.device)
    ...
    def forward(self, clm_embs: torch.Tensor, claim_idx: int) -> torch.Tensor:
        ...
        idxs1 = torch.where(torch.arange(n_clms).to(self.device) > claim_idx, 1.0, 0.0).unsqueeze(-1)
        idxs2 = torch.where(torch.arange(n_clms).to(self.device) == claim_idx, 1.0, 0.0).unsqueeze(-1)
        x = torch.cat([clm_embs, idxs1, idxs2], dim=-1)
        x = self.fc(x)
        x = self.attention(x)
